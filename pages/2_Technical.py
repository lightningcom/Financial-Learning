...
def load_module(filepath):
    try:
        ...
        spec = importlib.util.spec_from_file_location("dynamic_chapter_tech", filepath)
...
def technical():
    ...
    with st.sidebar:
        ...
        if not os.path.exists(folder_path):
            ...
            folder_path = os.path.join("streamlit", "Technical")
            if not os.path.exists(folder_path):
                 st.error(f"Could not find 'Technical' folder.")
                 return
        ...
        if "tech_chapter_selector" not in st.session_state:
            st.session_state.tech_chapter_selector = chapter_names[0]

        # Ensure the current selection is valid
        if st.session_state.tech_chapter_selector not in chapter_names:
             st.session_state.tech_chapter_selector = chapter_names[0]

        # Create text-like buttons for each chapter

        for chapter in chapter_names:
            if st.session_state.tech_chapter_selector == chapter:
                # Active Item
                st.button(chapter, key=f"btn_{chapter}", disabled=True, use_container_width=True)
            else:
                # Inactive Item (Clickable)
                if st.button(chapter, key=f"btn_{chapter}", use_container_width=True):
                    st.session_state.tech_chapter_selector = chapter
                    st.rerun()
        
        selected_chapter_name = st.session_state.tech_chapter_selector
        ...
        if current_index < len(chapter_names) - 1:
            with col3:
                def go_to_next_chapter():
                    curr_idx = chapter_names.index(st.session_state.tech_chapter_selector)
                    if curr_idx < len(chapter_names) - 1:
                        st.session_state.tech_chapter_selector = chapter_names[curr_idx + 1]
                
                st.button("Next Chapter →", on_click=go_to_next_chapter)
